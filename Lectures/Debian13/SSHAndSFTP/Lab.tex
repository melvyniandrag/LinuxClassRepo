...
chown user:group file\_or\_directory
...
To change permissions you do 
chmod xyz file\_or\_dir
...
The last digit is for everyone else on the computer
...
For directories its a bit different
4 - 
2 - 
1- 

For more info on permissions, see this link
\url{https://github.com/melvyniandrag/LinuxClassRepo/blob/master/usersAndPermissions.md}

\section{SSH Configuration}
\subsection{How SSH Works}
There's no time for the full story, but here's a little information. On your client machine (e.g. your laptop, pc, macbook, etc) you have a folder \textbf{\$HOME/.ssh} and in there you have an ssh keypair. In previous lectures we have used \textbf{ssh-keygen} to generate the key pair. One has file extension\textbf{.pub} and the other does not.

\begin{itemize}
\item id\_rsa ( or id\_ed22519, etc. ) is your private key
\item id\_rsa.pub ( or another name, it is your private key with a .pub added) is your public key
\end{itemize}

TODO add image here of output of ls $\sim$/.ssh/
...
$\sim$/.ssh/authorized\_keys

The authorized\_keys file can contain one public key for line for each public key that allows logging in.

So if the pub key on my laptop is 

"id\_rsa asdlgkjaheliuyth melvyn@pc"

and I want to ssh into my droplet like ssh root@192.334.223.445

then there must be a line in the file /root/.ssh/authorized\_keys that contains my key.

If I want to ssh in to melvyn@374.234.323.456, then on machine 374.234.323.456, my public key must be a line in the file 
/home/melvyn/.ssh/authorized\_keys
